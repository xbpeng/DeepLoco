...
cOptimizer::tOptParams::tOptParams()
{
	mOutputFile = "";
	mIntOutputPath = "";
	mEnableOutputLog = false;
}
...
// IO stuff
void cOptimizer::OutputPtInt(const cOptFunc::tPoint& pt, double obj_val, const std::string& filename_noext) const
{
	char suffix[64];
	sprintf(suffix, "_%.10f.txt", obj_val);
	std::string filename = filename_noext + suffix;
	OutputPt(pt, filename);
}

void cOptimizer::OutputPt(const cOptFunc::tPoint& pt, const std::string& filename) const
{
